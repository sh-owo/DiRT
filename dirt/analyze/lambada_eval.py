...
def run(
    ...
    return {
        ...
        "std_nll_dirt": float(np.std(dirt_target_nlls, ddof=1)) if n > 1 else 0.0,
        "std_nll_base": float(np.std(base_target_nlls, ddof=1)) if n > 1 else 0.0,
        "min_nll_dirt": float(np.min(dirt_target_nlls)) if n > 0 else 0.0,
        "max_nll_dirt": float(np.max(dirt_target_nlls)) if n > 0 else 0.0,
        "median_nll_dirt": float(np.median(dirt_target_nlls)) if n > 0 else 0.0,
        "min_nll_base": float(np.min(base_target_nlls)) if n > 0 else 0.0,
        "max_nll_base": float(np.max(base_target_nlls)) if n > 0 else 0.0,
        "median_nll_base": float(np.median(base_target_nlls)) if n > 0 else 0.0,
        "mean_diff": mean_diff,
        "std_diff": std_diff,
        "min_diff": float(np.min(diffs)) if n > 0 else 0.0,
        "max_diff": float(np.max(diffs)) if n > 0 else 0.0,
        "median_diff": float(np.median(diffs)) if n > 0 else 0.0,
        "win_count": win_count,
        "n_samples": n,
