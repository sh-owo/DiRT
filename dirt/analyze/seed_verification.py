...
def run_aggregate(results_by_seed: dict[int, dict], output_dir: Path) -> dict:
    ...
    return {
        ...
        "mean_diff": mean_diff,
        "std_diff": std_diff,
        "min_dirt_loss": float(np.min(dirt_losses)) if len(dirt_losses) > 0 else 0.0,
        "max_dirt_loss": float(np.max(dirt_losses)) if len(dirt_losses) > 0 else 0.0,
        "median_dirt_loss": float(np.median(dirt_losses)) if len(dirt_losses) > 0 else 0.0,
        "min_base_loss": float(np.min(base_losses)) if len(base_losses) > 0 else 0.0,
        "max_base_loss": float(np.max(base_losses)) if len(base_losses) > 0 else 0.0,
        "median_base_loss": float(np.median(base_losses)) if len(base_losses) > 0 else 0.0,
