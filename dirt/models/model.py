...
class DiRTModel(nn.Module):
    ...
    def _aggregate_metrics(self, all_metrics: list[dict[str, jnp.ndarray]]) -> dict[str, jnp.ndarray]:
        stacked = {k: jnp.stack([m[k] for m in all_metrics]) for k in ["delta_v", "imp_review", "magnitude_mean", "scaled_magnitude_mean", "review", "out"]}
        return {
            "avg_delta_v": jnp.mean(stacked["delta_v"]),
            "avg_imp_review": jnp.mean(stacked["imp_review"]),
            "avg_magnitude_mean": jnp.mean(stacked["magnitude_mean"]),
            "avg_scaled_magnitude_mean": jnp.mean(stacked["scaled_magnitude_mean"]),
